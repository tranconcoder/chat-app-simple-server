...
const account_schema_db_1 = __importDefault(
	require('../database/schema/account.schema.db')
);
const auth_schema_db_1 = __importDefault(
	require('../database/schema/auth.schema.db')
);
...
class Auth {
		...
		return __awaiter(this, void 0, void 0, function* () {
			try {
				...
				const validateResult = validator.validate(
					validateSchema_config_1.googleLoginSchema,
					req.user || {}
				);
	...
	checkUsernameIsExisted(req, res) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			const usernameIsUsed =
				yield account_schema_db_1.default.usernameIsUsed(username);
	...
	register(req, res) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			const validateResult = validator.validate(
				data,
				validateSchema_config_1.registerSchema
			);
			if (!validateResult.valid) {
			...
			const usernameIsUsed = yield newAccount.usernameIsUsed(
				data.username
			);
			if (usernameIsUsed)
				return res.status(401).send('Username is used!').end();
			account_schema_db_1.default.create(
	...
	login(req, res) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			const validateResult = validator.validate(
				data,
				validateSchema_config_1.loginSchema
			);
			if (!validateResult.valid) {
				return res.status(400).send('Data invalid!').end();
			}
			...
			const { userId } =
				(yield account_schema_db_1.default.findOne({
					username,
					password,
				})) || {};
			...
			const accessToken =
				(0, token_utils_1.generateToken)(
					authInfo._doc || authInfo,
					'access'
				) || '';
			const refreshToken =
				(0, token_utils_1.generateToken)(
